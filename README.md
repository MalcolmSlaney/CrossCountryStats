...
over multiple years and courses,
...
There is an individual runner_ability parameter for each runner, and as well as an
...
As part of this model we also estimate the average monthly and 
yearly improvement of each runner. Runners typically improve month over month
...
We have a multiplicate model and in effect we are multiplying A x B x C to predict D.
...
distribution of each model parameter. Our model has three point variables 
...
model parameter (this took about 4 hours for each gender's data).
